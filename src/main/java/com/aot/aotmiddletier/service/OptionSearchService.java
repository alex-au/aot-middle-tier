...
public class OptionSearchService {
	...
	private List<Future<String>> sendRequest() {
		...
		for (int i = 0; i < 25; i++) {
			callable = new OptionSerachTask(
					new OptionSearchService.SearchParam(i));
			collection.add(callable);
		}

		List<Future<String>> results = null;
		try {
			results = (List<Future<String>>) taskExecutor
					.getThreadPoolExecutor().invokeAll(collection, 10,
							TimeUnit.SECONDS);
